$(document).ready(function($) {
	...
	$(document).ready(function($) {
		...
		$(".st-default-search-input").keydown(function(event) {
			if (event.keyCode == 13) {
				$.cookie('dawn-search', this.value, {
					expires: 7,
					path: '/'
				})
		...
		//当滚动条的位置处于距顶部2800像素以下时，跳转链接出现，否则消失  
		$(function() {
			$(window).scroll(function() {
		...
		$("div>table").wrap("<div class='table-area'></div>");
...
$(document).ready(function($) {
	...
	$.each($('.toc-level-2').has('.toc-child'),function(i,tocL2){
		$(tocL2).children(".toc-link").after("<a class='tocTrigger' href='javascript:;'><span class='sb-caret'></span></a>");
	});
	$.each($('.toc-level-1').has('.toc-child'),function(i,tocL1){
		$(tocL1).children(".toc-link").after("<a class='tocTrigger' href='javascript:;'><span class='sb-caret'></span></a>");
	});
	$.each($('.toc-level-3').has('.toc-child'),function(i,tocL1){
		$(tocL1).children(".toc-link").after("<a class='tocTrigger' href='javascript:;'><span class='sb-caret'></span></a>");
	});
	$('.tocTrigger').off('click')
		.on('click', function() {
			$submenu = $(this).parent().children('.toc-child');
			$(this).add($submenu).toggleClass('sb-submenu-active'); // Toggle active class.
